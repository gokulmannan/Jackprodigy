...
@Component({
  selector: 'app-edit-naac-file-popup',
  templateUrl: './edit-naac-file-popup.component.html',
  styleUrls: ['./edit-naac-file-popup.component.css']
})
export class EditNaacFilePopupComponent implements OnInit {
  @Input() naacId:any;
  @Input() fileLabel:any;
...
removeFile(fileId) {
      this.naac.fieldValues[this.fieldFile]["fileKey"] = null;
      this.naac.fieldValues[this.fieldFile]["url"] = null;
...
deleteFile(key) {
  this.naacService.deleteFile(this.naac.fields[this.fieldFile].versionId, this.naac.fields[this.fieldFile].fileKey).then((data: any) => {
    this.naac.fields[this.fieldFile] = undefined;
    this.naacService.updateNaac(this.naac).then((data: any) => {
      this.getNaacByIdWithLabel();
    });
  })
